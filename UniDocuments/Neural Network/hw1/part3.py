...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        #TODO: error mismatch  m1: [1792 x 28], m2: [50176 x 256]
        #x = x.view(x.size(0), -1)
...
class NNModel:
    def __init__(self, network, learning_rate):
        ...
        """
        #TODO: loss function for cross entropy with softmax
    ...
    def view_batch(self):
        ...
        #     print(i)
        #print(list(self.trainloader))
        pic1 = iter(self.trainloader)
        pic1 = next(pic1)
        images = pic1[0]
        #print(images.size())
        #image1 = torch.cat(images for i in range(0,8)))
        #images = images.view(28,8,28,8)
        #print(images)
        #remove -1
        images = torch.squeeze(images)
        #print(images.size())
        #print(images)
        #change 1 and 2 dimension
        images = images.permute(0, 2, 1)
        #outputs = images.view(28 * 8, 28 * 8)
        #concatnate 0-8 8-16 ----- 64
        #print("im1", images.size())
        images = images.reshape(8, 28 * 8, 28)
        #print("im2", images.size())
        images = images.permute(0, 2, 1)
        #print("im3",images.size())
        images = images.reshape(28 * 8, 28 * 8)
        #print("im4",images.size())
        images = images.numpy()
        labels = pic1[1]
        labels = labels.view(8,8)
        labels = labels.numpy()
        #temp1 = [8elemennt]
        #[[8 element],[8 element,[8 element].....]
