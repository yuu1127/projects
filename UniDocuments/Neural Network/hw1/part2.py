...
class LinearModel:
    ...
    def loss(prediction, label):
        ...
        print(prediction)
        e = -1 * label * np.log(prediction) - (1 - label) * np.log(1 - prediction)
        return e
    ...
    def backward(self, inputs, diff):
        ...
        # if diff > 0:
        #     for i in range(len(self.weights) - 1):
        #         self.weights[i] = self.weights[i] - self.lr * diff
        #     self.weights[-1] = self.weights[-1] - self.lr
        # elif diff < 0:
        #     for j in range(len(self.weights) - 1):
        #         self.weights[j] = self.weights[j] - self.lr * diff
        #     self.weights[-1] = self.weights[-1] + self.lr
        print(diff)
        print(inputs)
        for i in range(len(self.weights) - 1):
            self.weights[i] = self.weights[i] + self.lr * diff * inputs[i]
